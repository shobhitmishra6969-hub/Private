...
module.exports = {
  ...
  async execute(message, args, client, prefix) {
    ...
    const songHeader = new TextDisplayBuilder()
      .setContent(`**${emoji.check} [${song.title}](${song.uri})**`);

    const separator = new SeparatorBuilder();

    const songInfoSection = new SectionBuilder()
      .addTextDisplayComponents(
        (textDisplay) => textDisplay.setContent(`**${emoji.info} Duration:** \`${convertTime(total)}\``),
        (textDisplay) => textDisplay.setContent(`**${emoji.info} Author:** \`${cleanAuthorName(song.author)}\``)
      );
