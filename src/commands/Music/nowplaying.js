...
module.exports = {
  ...
  async execute(message, args, client, prefix) {
    ...
    const getSourceEmoji = (uri = '') => {
      if (uri.includes('spotify.com'))         return emoji.spotify;
      if (uri.includes('music.youtube.com'))   return emoji.ytmusic;
    ...
    const createContainer = (prog) => {
      const sourceEmoji = getSourceEmoji(track.uri);

      const headerDisplay = new TextDisplayBuilder()
        .setContent(`${emoji.dance} **Now Playing**\n${emoji.dot} [${track.title}](${track.uri})`);

      const separator1 = new SeparatorBuilder();

      const infoDisplay = new TextDisplayBuilder()
        .setContent(
          `${emoji.dot} **Artist:** [${cleanAuthorName(track.author)}](${track.uri})\n` +
          `${emoji.dot} **Duration:** \`${durationFormatted}\`\n` +
          `${emoji.dot} **Source:** ${sourceEmoji}\n` +
          `${emoji.dot} **Requester:** [${track.requester.username}](https://discord.com/users/${track.requester.id})\n\n` +
          `${emoji.play} \`${prog.position}\` \`${prog.bar}\` \`${durationFormatted}\``
        );
