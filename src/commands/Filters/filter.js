    ...
    async execute(message, args, client, prefix) {
        ...
        if (!player.queue.current) {
            const warnDisplay = new TextDisplayBuilder()
                .setContent(`**${emoji.warn} There is no song currently playing.**`);
        ...
        const row4 = new ActionRowBuilder().addComponents(
            new StringSelectMenuBuilder()
                ...
                .addOptions([
                    { label: "Reset Filters", value: "clear", emoji: emoji.warn },
                    { label: "8D Audio", value: "8d_but", emoji: emoji.check },
                    { label: "BassBoost", value: "bass_but", emoji: emoji.check },
                    { label: "Deep Bass", value: "deepbass_but", emoji: emoji.check },
                    { label: "Treble Boost", value: "treble_but", emoji: emoji.check },
                    { label: "NightCore", value: "night_but", emoji: emoji.check },
                    { label: "Daycore", value: "daycore_but", emoji: emoji.check },
                    { label: "Slowed + Reverb", value: "slowed_but", emoji: emoji.check },
                    { label: "Vaporwave", value: "vapo_but", emoji: emoji.check },
                    { label: "Chipmunk", value: "chipmunk_but", emoji: emoji.check },
                    { label: "Karaoke", value: "karaoke_but", emoji: emoji.check },
        ...
        const statusDisplay = new TextDisplayBuilder()
            .setContent(`**${emoji.info} Current Filter** \`:\` \`${currentFilter}\``);
            ...
            filter: (i) => {
                if (message.author.id === i.user.id) return true;
                    ...
                    const errorDisplay = new TextDisplayBuilder()
                        .setContent(`**${emoji.console.warn} That's not your session run. Use \`${prefix}filter\` to create your own.**`);
        ...
        collector.on("collect", async (i) => {
            ...
            const updatedDisplay = new TextDisplayBuilder()
                .setContent(`**${emoji.info} Current Filter \`:\`** \`${filterName}\``);
